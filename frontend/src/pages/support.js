...
export async function renderSupport() {
  ...
  const [{ data: tickets }, { data: adminConfig }] = await Promise.all([
    supabase.from('tickets').select('*').eq('company_id', companyId).order('created_at', { ascending: false }),
    supabase.from('admin_config').select('whatsapp_number').single(),
  ]);

  const whatsappNumber = adminConfig?.whatsapp_number || '573000000000';
  const waMsg = encodeURIComponent(`Hola! Soy ${profile.name || profile.email} de ${profile.companies?.name || 'mi empresa'}. Necesito ayuda con CotiCuenta.`);
    ...
    <div class="app-layout">
      ...
      <div class="main-content">
        ...
        <div class="page-content">
          ...
          <a href="https://wa.me/${whatsappNumber}?text=${waMsg}" target="_blank" class="btn btn-block btn-lg" style="background:#25d366;border-color:#25d366;color:#fff;margin-bottom:20px;display:flex;align-items:center;justify-content:center;gap:8px">
            ...
            ¿Necesitas ayuda rápida? Escríbenos por WhatsApp
          </a>
